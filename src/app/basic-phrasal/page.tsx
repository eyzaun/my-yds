...
export default function TemelPhrasalVerbsPage() {
  const { tokens } = useTheme();
  const { user } = useAuth();

  const categoryId = 'basic-phrasal';
  const categoryName = 'Temel Phrasal Verbs';
  const categoryKey = 'basicPhrasal' as keyof typeof vocabulary;
  ...
  useEffect(() => {
    ...
    script.innerHTML = JSON.stringify({
      ...
      '@type': 'WebPage',
      'name': 'YDS Temel Phrasal Verbs Kelime Listesi',
      'description': 'YDS için temel düzey phrasal verbs.',
    ...
    <div className="min-h-screen pb-16" style={{ backgroundColor: tokens.colors.background.primary }}>
      <Container maxWidth="lg" className="py-8">
        ...
        <Card variant="elevated" className="mb-6">
          <Text className="mb-3">
            YDS için temel düzey phrasal verbs.
          </Text>
        ...
        <Card variant="elevated" className="mt-8">
          <Heading3 className="mb-3">
            Temel Phrasal Verbs Hakkında
          </Heading3>
          <Text>
            YDS için temel düzey phrasal verbs. Bu kelimeler YDS sınavında sıkça karşınıza çıkabilir.
          </Text>
        </Card>
      </Container>
      
      <ClientOnlyAd 
        slot="basic-phrasal-footer"
        format="auto"
