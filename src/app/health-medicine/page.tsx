...
export default function SağlıkveTıpPage() {
  const { tokens } = useTheme();
  const { user } = useAuth();

  const categoryId = 'health-medicine';
  const categoryName = 'Sağlık ve Tıp';
  const categoryKey = 'healthMedicine' as keyof typeof vocabulary;
  ...
  useEffect(() => {
    ...
    script.innerHTML = JSON.stringify({
      ...
      '@type': 'WebPage',
      'name': 'YDS Sağlık ve Tıp Kelime Listesi',
      'description': 'Sağlık, tıp ve medikal terimler.',
    ...
    <div className="min-h-screen pb-16" style={{ backgroundColor: tokens.colors.background.primary }}>
      <Container maxWidth="lg" className="py-8">
        ...
        <Card variant="elevated" className="mb-6">
          <Text className="mb-3">
            Sağlık, tıp ve medikal terimler.
          </Text>
        ...
        <Card variant="elevated" className="mt-8">
          <Heading3 className="mb-3">
            Sağlık ve Tıp Hakkında
          </Heading3>
          <Text>
            Sağlık, tıp ve medikal terimler. Bu kelimeler YDS sınavında sıkça karşınıza çıkabilir.
          </Text>
        </Card>
      </Container>
      
      <ClientOnlyAd 
        slot="health-medicine-footer"
        format="auto"
