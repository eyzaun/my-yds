...
export default function HedefPhrasalVerbsPage() {
  const { tokens } = useTheme();
  const { user } = useAuth();

  const categoryId = 'target-phrasal';
  const categoryName = 'Hedef Phrasal Verbs';
  const categoryKey = 'targetPhrasal' as keyof typeof vocabulary;
  ...
  useEffect(() => {
    ...
    script.innerHTML = JSON.stringify({
      ...
      '@type': 'WebPage',
      'name': 'YDS Hedef Phrasal Verbs Kelime Listesi',
      'description': 'YDS sınavında hedeflenen önemli phrasal verbs.',
    ...
    <div className="min-h-screen pb-16" style={{ backgroundColor: tokens.colors.background.primary }}>
      <Container maxWidth="lg" className="py-8">
        ...
        <Card variant="elevated" className="mb-6">
          <Text className="mb-3">
            YDS sınavında hedeflenen önemli phrasal verbs.
          </Text>
        ...
        <Card variant="elevated" className="mt-8">
          <Heading3 className="mb-3">
            Hedef Phrasal Verbs Hakkında
          </Heading3>
          <Text>
            YDS sınavında hedeflenen önemli phrasal verbs. Bu kelimeler YDS sınavında sıkça karşınıza çıkabilir.
          </Text>
        </Card>
      </Container>
      
      <ClientOnlyAd 
        slot="target-phrasal-footer"
        format="auto"
