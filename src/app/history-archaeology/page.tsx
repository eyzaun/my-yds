...
export default function TarihveArkeolojiPage() {
  const { tokens } = useTheme();
  const { user } = useAuth();

  const categoryId = 'history-archaeology';
  const categoryName = 'Tarih ve Arkeoloji';
  const categoryKey = 'historyArchaeology' as keyof typeof vocabulary;
  ...
  useEffect(() => {
    ...
    script.innerHTML = JSON.stringify({
      ...
      '@type': 'WebPage',
      'name': 'YDS Tarih ve Arkeoloji Kelime Listesi',
      'description': 'Tarih, arkeoloji ve kültür ile ilgili terimler.',
    ...
    <div className="min-h-screen pb-16" style={{ backgroundColor: tokens.colors.background.primary }}>
      <Container maxWidth="lg" className="py-8">
        ...
        <Card variant="elevated" className="mb-6">
          <Text className="mb-3">
            Tarih, arkeoloji ve kültür ile ilgili terimler.
          </Text>
        ...
        <Card variant="elevated" className="mt-8">
          <Heading3 className="mb-3">
            Tarih ve Arkeoloji Hakkında
          </Heading3>
          <Text>
            Tarih, arkeoloji ve kültür ile ilgili terimler. Bu kelimeler YDS sınavında sıkça karşınıza çıkabilir.
          </Text>
        </Card>
      </Container>
      
      <ClientOnlyAd 
        slot="history-archaeology-footer"
        format="auto"
