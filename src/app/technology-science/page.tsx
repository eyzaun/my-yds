...
export default function TeknolojiveBilimPage() {
  const { tokens } = useTheme();
  const { user } = useAuth();

  const categoryId = 'technology-science';
  const categoryName = 'Teknoloji ve Bilim';
  const categoryKey = 'technologyScience' as keyof typeof vocabulary;
  ...
  useEffect(() => {
    ...
    script.innerHTML = JSON.stringify({
      ...
      '@type': 'WebPage',
      'name': 'YDS Teknoloji ve Bilim Kelime Listesi',
      'description': 'Teknoloji, bilim ve inovasyon alanlarındaki terimler.',
    ...
    <div className="min-h-screen pb-16" style={{ backgroundColor: tokens.colors.background.primary }}>
      <Container maxWidth="lg" className="py-8">
        ...
        <Card variant="elevated" className="mb-6">
          <Text className="mb-3">
            Teknoloji, bilim ve inovasyon alanlarındaki terimler.
          </Text>
        ...
        <Card variant="elevated" className="mt-8">
          <Heading3 className="mb-3">
            Teknoloji ve Bilim Hakkında
          </Heading3>
          <Text>
            Teknoloji, bilim ve inovasyon alanlarındaki terimler. Bu kelimeler YDS sınavında sıkça karşınıza çıkabilir.
          </Text>
        </Card>
      </Container>
      
      <ClientOnlyAd 
        slot="technology-science-footer"
        format="auto"
