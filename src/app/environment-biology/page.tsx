...
export default function ÇevreveBiyolojiPage() {
  const { tokens } = useTheme();
  const { user } = useAuth();

  const categoryId = 'environment-biology';
  const categoryName = 'Çevre ve Biyoloji';
  const categoryKey = 'environment_and_biology' as keyof typeof vocabulary;
  ...
  useEffect(() => {
    ...
    script.innerHTML = JSON.stringify({
      ...
      '@type': 'WebPage',
      'name': 'YDS Çevre ve Biyoloji Kelime Listesi',
      'description': 'Çevre, ekoloji ve biyoloji ile ilgili terimler.',
    ...
    <div className="min-h-screen pb-16" style={{ backgroundColor: tokens.colors.background.primary }}>
      <Container maxWidth="lg" className="py-8">
        ...
        <Card variant="elevated" className="mb-6">
          <Text className="mb-3">
            Çevre, ekoloji ve biyoloji ile ilgili terimler.
          </Text>
        ...
        <Card variant="elevated" className="mt-8">
          <Heading3 className="mb-3">
            Çevre ve Biyoloji Hakkında
          </Heading3>
          <Text>
            Çevre, ekoloji ve biyoloji ile ilgili terimler. Bu kelimeler YDS sınavında sıkça karşınıza çıkabilir.
          </Text>
        </Card>
      </Container>
      
      <ClientOnlyAd 
        slot="environment-biology-footer"
        format="auto"
